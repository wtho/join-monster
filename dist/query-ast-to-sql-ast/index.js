...
function populateASTNode(queryASTNode, parentTypeNode, sqlASTNode, namespace, depth, options, context) {
  ...
  if (!field.jmIgnoreTable && TABLE_TYPES.includes(gqlType.constructor.name) && config.sqlTable) {
    ...
    const dependencyASTNode = _extends({}, queryASTNode, {
      kind: 'Field',
      name: _extends({}, queryASTNode.name, { value: field.directDependency.name }),
      selectionSet: field.directDependency.fields ? {
        kind: 'SelectionSet',
        loc: queryASTNode.loc,
        selections: field.directDependency.fields.map(siblingName => ({
          alias: undefined,
          args: [],
          directives: [],
          kind: 'Field',
          loc: queryASTNode.loc,
          name: { kind: 'name', loc: queryASTNode.loc, value: siblingName, selectionSet: undefined }
        }))
      } : undefined
    });
